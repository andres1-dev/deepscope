...
async function handleActualizarDatosSubmit(e) {
    ...
    // 1. Actualizar objeto en memoria — la UI se sincroniza al instante
    if (window.currentUser && window.currentUser.ROL === 'GUEST') {
        window.currentUser.EMAIL        = emailPlanta;
        window.currentUser.TELEFONO     = rawTelefono;
        window.currentUser.DIRECCION    = direccion;
        window.currentUser.DEPARTAMENTO = departamento;
        window.currentUser.CIUDAD       = ciudad;
        window.currentUser.BARRIO       = barrio;
    }

...
async function _subirArchivoDrive(fileData, id, hoja) {
    console.log(`[UPLOAD] Iniciando subida a Supabase (Bucket: soportes-r2) | Hoja: ${hoja}`);
    ...
    try {
        ...
        const publicUrl = await window.uploadToSupabase(file);
        ...
        if (hojaUpper === 'NOVEDADES' || hojaUpper === 'REPORTES') {
            ...
            await fetch(`${CONFIG.FUNCTIONS_URL}/operations`, {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                    'Authorization': `Bearer ${SUPABASE_KEY}`,

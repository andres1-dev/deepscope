...
function createFloatingCSVButton() {
  ...
  mainBtn.id = 'floating-csv-btn-main';
  mainBtn.title = 'Actualizar';
  mainBtn.style.cssText = `
    ...
    outline: none;
  `;
  mainBtn.innerHTML = `<i class="fas fa-file-import"></i>`;
  ...
  mainBtn.onmouseover = () => {
    mainBtn.style.transform = 'scale(1.1) translateY(-2px)';
    mainBtn.style.boxShadow = '0 12px 28px rgba(63,81,181,0.5)';
  };
  mainBtn.onmouseout = () => {
    mainBtn.style.transform = 'scale(1) translateY(0)';
    mainBtn.style.boxShadow = '0 8px 24px rgba(63,81,181,0.4)';
  };
        ...
        <div style="display:flex; align-items:center; gap:14px;">
          ...
          <div>
            ...
            <p style="margin:0; font-size:0.8rem; color:#64748b; font-weight:400; text-transform:none;">
              Carga masiva a Supabase · BUSINT
            </p>
          </div>
        </div>
...
async function processCSVUpload() {
  ...
  try {
    ...
    const response = await fetch(`${CONFIG.FUNCTIONS_URL}/operations`, {
      ...
      body: JSON.stringify({ accion: 'SYNC_BUSINT', records: csvData })
    });
      ...
      resultDiv.innerHTML = `
        ...
        ">
          ...
          <div style="color: #166534; font-size: 0.875rem;">
            ...
            BUSINT actualizado: ${uploaded} registros cargados.
          </div>
        </div>
      `;
    } else {
    ...
    if (uploaded > 0 && typeof loadBUSINT === 'function') {
      setTimeout(() => loadBUSINT(), 2000);
    }

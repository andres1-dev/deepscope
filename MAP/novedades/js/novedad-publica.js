/* novedad-publica.js - Formulario p�blico de novedades */

// ---------------------------------------------------------------------------
// PREVENIR CACH� - FORZAR DATOS FRESCOS SIEMPRE
// ---------------------------------------------------------------------------
...
function formatearFechaLarga(fechaStr) {
    try {
        ...
        // Calcular d�as h�biles desde la fecha hasta hoy
        const hoy = new Date();
        ...
        // Determinar color seg�n d�as h�biles
        let colorClass = '';
        let textoTiempo = '';
        ...
        if (diasHabiles === 0) {
            ...
            textoTiempo = 'hace 1 d�a h�bil';
            colorClass = 'fecha-verde';
        } else if (diasHabiles === 2) {
            textoTiempo = 'hace 2 d�as h�biles';
            colorClass = 'fecha-verde';
        } else if (diasHabiles > 2) {
            textoTiempo = `hace ${diasHabiles} d�as h�biles`;
            colorClass = 'fecha-rojo';
        } else {
            // Fecha futura
            const diasFuturos = Math.abs(diasHabiles);
            if (diasFuturos === 1) {
                textoTiempo = 'en 1 d�a h�bil';
            } else {
                textoTiempo = `en ${diasFuturos} d�as h�biles`;
            }
            colorClass = 'fecha-verde';
        }
        
        return `${diaSemana}, ${dia} de ${mes} del ${a�o} <span class="${colorClass}">(${textoTiempo})</span>`;
    } catch (error) {
...
function calcularDiasHabiles(fechaInicio, fechaFin) {
    // Si fechaInicio es despu�s de fechaFin, invertir y retornar negativo
    let invertido = false;
...
function continuarAdicional() {
    // Validar que se haya seleccionado un �rea
    const area = document.getElementById('area').value;
    if (!area) {
        showError(document.getElementById('area'), document.getElementById('areaError'), 'Por favor selecciona un �rea');
        return;
    }
    ...
    // Validar campos din�micos seg�n el �rea
    if (area === 'INSUMOS') {
    ...
    } else if (area !== 'DISE�O' && area !== 'TELAS') {
        // Para �reas con cantidad normal (OTROS)
...
function _crearFilaDinamica(opciones, listId, removeFn) {
    ...
    if (!lista) {
        console.error('[_crearFilaDinamica] ERROR: No se encontr� el elemento con ID:', listId);
        return null;
    }
    ...
    if (listId.includes('corte')) {
        ...
        labelTipo = 'Tipo de Imperfecci�n';
        iconoTipo = 'fa-scroll';
        placeholder = 'Escribe o selecciona imperfecci�n...';
...
function handleAreaChange(e) {
    ...
    if (!tipoGroup) {
        console.error('[handleAreaChange] ERROR: No se encontr� tipoNovedadGroup');
        return;
    }
    ...
    if (area === 'DISE�O') {
        ...
        unlockAllFields(); // Dise�o no tiene m�s campos
        ...
        unlockAllFields(); // Telas ya mostr� todos sus campos
...
function handleTipoNovedadChange(e) {
    ...
    // Mostrar el grupo correspondiente seg�n el �rea
...
// Funciones auxiliares para revelaci�n progresiva
...
function lockFieldsAfter(fieldId) {
    // Esta funci�n bloquea visualmente los campos que vienen despu�s del especificado
...
async function cargarCurvaParaCodigos() {
    ...
    if (!op) {
        Swal.fire({ title: 'Error', text: 'No se encontr� la OP del producto.', icon: 'error', confirmButtonColor: '#673ab7' });
        return;
    }
    try {
        if (CURVAS_CACHE[op]) {
        ...
        if (!curva || !curva.detalles || curva.detalles.length === 0) {
            Swal.fire({ title: 'Sin curva', html: `No se encontr� curva para la OP: <strong>${op}</strong>`, icon: 'warning', confirmButtonColor: '#673ab7' });
            return;
        }
        CURVAS_CACHE[op] = curva;
        poblarCodigosDesdeDetalles(curva.detalles);
    } catch (error) {
        console.error('[c�digos] Error:', error);
...
function agregarFilaCodigo() {
    ...
    fila.innerHTML = `
        ...
        <div class="form-group" style="margin-bottom: 0;">
            ...
            <div class="input-wrapper">
                <i class="fas fa-hashtag input-icon"></i>
                <input type="number" class="form-control codigo-cantidad" min="1" placeholder="M�x: -" required>
            </div>
        </div>
...
function actualizarMaximoCodigo(selectElement) {
    ...
    // Validaci�n en tiempo real al cambiar
    if (talla) {
        tallaSelect.classList.remove('error');
    ...
    if (detalle) {
        ...
        inputCantidad.placeholder = `M�x: ${maximo}`;
    } else {
        inputCantidad.placeholder = 'M�x: -';
        inputCantidad.max = '';
    }
...
function _recolectarFilas(listId) {
    ...
    filas.forEach(fila => {
        ...
        // Obtener valor y convertir a may�sculas para env�o
        ...
        // Convertir a may�sculas solo para el env�o
...
function validateAndPreviewFile(file) {
    ...
    if (!ValidationRules.imagen.allowedTypes.includes(file.type)) {
        showError(input, errorElement, 'Solo se permiten im�genes JPG, PNG o GIF');
        input.value = '';
        return;
    }
...
function validateForm() {
    ...
    // Descripci�n es opcional, solo validar si tiene contenido
    const descripcion = document.getElementById('descripcion').value.trim();
    if (descripcion.length > 0 && descripcion.length < 10) {
        showError(document.getElementById('descripcion'), document.getElementById('descripcionError'), 'La descripci�n debe tener al menos 10 caracteres');
        errors.push('Descripci�n muy corta');
        isValid = false;
    }
...
async function handleSubmit(e) {
    ...
    const result = await Swal.fire({
        icon: 'question', title: '�Enviar reporte?', text: 'Verifica que toda la informaci�n sea correcta',
    ...
    try {
        ...
        if (response.success) {
            // Mostrar pantalla de �xito estilo Google Forms
